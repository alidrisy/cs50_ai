...
class MinesweeperAI:
    ...
    def add_knowledge(self, cell, count):
        ...
        while new_information:
            ...
            # Check for new safes and mines
            for sentence in self.knowledge:
                if sentence.known_safes() and not sentence.known_mines():
                    for safe in sentence.known_safes():
                        if safe not in self.safes:
                            self.mark_safe(safe)
                            new_information = True

                if sentence.known_mines() and not sentence.known_safes():
                    for mine in sentence.known_mines():
                        if mine not in self.mines:
                            self.mark_mine(mine)
                            new_information = True

            # Check for new sentences to add
            for i in range(len(self.knowledge)):
                for j in range(len(self.knowledge)):
                    if i != j and self.knowledge[i].cells.issubset(
